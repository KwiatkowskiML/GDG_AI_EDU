...
function MouthAnimationController({ blendshapeMesh, talking }: MouthAnimationControllerProps) {
  ...
  useFrame(() => {
    if (!talking || !blendshapeMesh || !blendshapeMesh.morphTargetDictionary || !(blendshapeMesh as any).morphTargetInfluences) {
      // If not talking or mesh not ready, ensure mouth is closed (or at its base state)
      if (blendshapeMesh && blendshapeMesh.morphTargetDictionary && (blendshapeMesh as any).morphTargetInfluences) {
        const mouthOpenIndex = blendshapeMesh.morphTargetDictionary['mouthOpen']; // Common name
        const jawOpenIndex = blendshapeMesh.morphTargetDictionary['jawOpen']; // Another common name
        const influences = (blendshapeMesh as any).morphTargetInfluences as number[];
        if (mouthOpenIndex !== undefined && influences[mouthOpenIndex] !== 0) influences[mouthOpenIndex] = 0;
        if (jawOpenIndex !== undefined && influences[jawOpenIndex] !== 0) influences[jawOpenIndex] = 0;
      }
    ...
    const targetIndex = jawOpenIndex !== undefined ? jawOpenIndex : mouthOpenIndex;
...
function LoadedModel({ modelUrl, onGltfRootLoad, onMeshIdentified }: LoadedModelProps) {
  const { scene: gltfRoot, parser } = useGLTF(modelUrl);
  const [identifiedMesh, setIdentifiedMesh] = useState<THREE.Mesh | THREE.SkinnedMesh | null>(null);
  ...
  useEffect(() => {
    ...
    const priorityMeshNames = ["Wolf3D_Head", "Wolf3D_Avatar", "RPM_Head", "RPM_Avatar", "head", "face"];
    ...
    gltfRoot.traverse((child) => {
      ...
      if (node.morphTargetDictionary) {
        if (node.isSkinnedMesh) {
          if (priorityMeshNames.some(name => node.name.toLowerCase().includes(name.toLowerCase()))) {
            if (!bestMatch || (bestMatch.name && bestMatch.name.toLowerCase().includes("teeth"))) {
              bestMatch = node as THREE.SkinnedMesh;
            }
          }
          if (!firstSkinnedMeshWithMorphs) firstSkinnedMeshWithMorphs = node as THREE.SkinnedMesh;
        } else if (node.isMesh) {
          if (priorityMeshNames.some(name => node.name.toLowerCase().includes(name.toLowerCase()))) {
    ...
    else if (firstSkinnedMeshWithMorphs) setIdentifiedMesh(firstSkinnedMeshWithMorphs);
    else if (firstMeshWithMorphs) setIdentifiedMesh(firstMeshWithMorphs);
    else { console.warn("No suitable mesh with morph targets found in the model."); /* ... */ }
  }, [gltfRoot, parser]);
  ...
  useEffect(() => {
    if (identifiedMesh && identifiedMesh.morphTargetDictionary && !identifiedMesh.morphTargetInfluences) {
        const numMorphs = Object.keys(identifiedMesh.morphTargetDictionary).length;
        (identifiedMesh as any).morphTargetInfluences = new Array(numMorphs).fill(0);
    }
  }, [identifiedMesh]);
...
const Model: React.FC = () => {
  const [blendshapeMesh, setBlendshapeMesh] = useState<THREE.Mesh | THREE.SkinnedMesh | null>(null);
  ...
  return (
    <div className="w-full h-full relative bg-transparent"> {/* Ensured transparent bg */}
      <button
        onClick={() => setIsTalking(!isTalking)}
        className="absolute top-4 right-4 z-[1001] text-white font-semibold py-2 px-4 rounded-lg shadow-md
                   transition-all duration-200 ease-in-out
                   focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-transparent
                   text-xs sm:text-sm
                   min-w-[120px] text-center /* Ensure button has some min width */
                   bg-sky-500 hover:bg-sky-600 focus:ring-sky-400" // Default: "Stop Talking" style
        // Conditional styling (example, not fully implemented here, could also change icon)
        // className={`... ${isTalking ? 'bg-red-500 hover:bg-red-600 focus:ring-red-400' : 'bg-green-500 hover:bg-green-600 focus:ring-green-400'}`}
      >
      ...
      >
        ...
        <Suspense fallback={
          ...
          />
          {blendshapeMesh && <MouthAnimationController blendshapeMesh={blendshapeMesh} talking={isTalking} />}
        </Suspense>
      </Canvas>
    </div>
  );
