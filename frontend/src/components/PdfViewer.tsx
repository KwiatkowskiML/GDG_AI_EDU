// PdfViewer.tsx
const PdfViewer = () => {
  const pdfPath = "/documents/attention_is_all_you_need.pdf"; 
  return (
    ...
    <div className="absolute top-16 left-1/5 w-4/5 h-[calc(100%-4rem)] flex flex-col items-center pt-2 pb-4 px-4"> {/* h-screen without navbar height */}
      ...
      <object
        ...
        data={pdfPath} // Make sure CORS allows this
        type="application/pdf"
      >
        <p className="p-4 text-slate-600">
          It appears your browser does not support embedding PDFs.
          You can <a href={pdfPath} className="text-sky-600 hover:underline">download the PDF</a> instead.
        </p>
      </object>
    </div>
  );
